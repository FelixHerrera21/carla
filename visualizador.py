...
lectura_pesos = input()
lectura_pesos = lectura_pesos[1:-1]
lectura_pesos = lectura_pesos.split(",")
lectura_pesos = np.array(lectura_pesos)
lectura_pesos = lectura_pesos.astype(np.float)
print(lectura_pesos)
print(type(lectura_pesos))

gbest_pos = lectura_pesos
funcion_Aptitud_Recta(gbest_pos, random.choice(world.get_map().get_spawn_points()))
