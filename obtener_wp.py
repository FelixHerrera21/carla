...
def obtenerWaypoints(world, distancia_entre_wp, cantidad_wp):
	map1 = world.get_map()
	...
	plt.plot(x, y, 'bo',  marker = '.', color = 'b')
	ruta = []
	...
	#plt.plot([wp_prueba.transform.location.x], [-wp_prueba.transform.location.y], 'bo',  marker = '.', color = 'r')

	next_wp = wp_prueba.next(distancia_entre_wp)

	plt.plot([next_wp[0].transform.location.x], [-next_wp[0].transform.location.y], 'bo',  marker = '.', color = 'g')

	for i in range(1,cantidad_wp):
		next_wp = next_wp[-1].next(distancia_entre_wp)
		...
		plt.plot([x], [-y], 'bo',  marker = '.', color = 'g')
...
distancia_entre_wp = 5
cantidad_wp = 100
mapa_json = obtenerWaypoints(world, distancia_entre_wp, cantidad_wp)
